...
emoji = pictures [
    ...
    (polyline [ ( 1.2 * cos a, 1.2 * sin a) | a <- [0, 0.02 .. 4 * pi/4] ]))
    ...
    (polyline [ ( 1.4 * cos a, 1.7 * sin a) | a <- [0, 0.02 .. 4 * pi/4] ]))
                ]
...
hand = pictures [
    ...
    (polyline [ ( 2.4 * cos a, 2.5 * sin a) | a <- [0, 0.02 .. 4 * pi/4] ]))
    , rotated (pi) ( translated (8) (3)
    (polyline [ ( - 0.8 *  cos a, -2.2 * sin a) | a <- [0, 0.02 .. 4 * pi/4] ]))
